...
@app.cell
def __(mo):
    mo.md("# OBEC-Specific Zero-Vote Analysis (2021 Election)")
    return


@app.cell
def __(edl):
    # Load 2021 election data using shared function
    df = edl.load_election_data(year=2021)
...
def __(go, mo, np, prob_analysis, px):
    ...
    geo_scatter = px.scatter(
        ...
        title='Zero-Vote Commissions: OBEC-Specific Analysis (Municipality Size vs Commission Size) - 2021',
        labels={
...
def __(go, make_subplots, mo, prob_analysis):
    ...
    fig.update_layout(
        title_text='Comparing Overall vs OBEC-Specific Probability Calculations (2021)',
        height=500,
        showlegend=False
    )
...
def __(mo, prob_analysis, px):
    ...
    improvement_fig = px.histogram(
        ...
        nbins=50,
        title='Distribution of Accuracy Improvement: Overall → OBEC-Specific (2021)',
        labels={
...
def __(mo, prob_analysis, px):
    ...
    support_fig = px.histogram(
        ...
        nbins=50,
        title='Distribution of Party Support in Municipality (for zero-vote commissions) - 2021',
        labels={
