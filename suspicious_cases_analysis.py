...
def __(mo, municipality_names, party_names, pd, zero_votes_with_prob):
    ...
    for _idx, _row in most_suspicious.iterrows():
        ...
        _party_name = party_names.get(_party_code, str(_party_code))
        _obec_code = _row['OBEC']
        _obec_name = municipality_names.get(_obec_code, f"Unknown ({_obec_code})")

        _status = 'HIGHLY SUSPICIOUS' if _row['Is_Highly_Suspicious'] else (
        ...
        summary_data.append({
            ...
            'Municipality': _obec_name,
            'OBEC_Code': _obec_code,
...
def __(mo, summary_df):
    ...
    for _i, _row_detail in summary_df.head(10).iterrows():
        print(f"\n{_row_detail['Rank']}. {_row_detail['Party']} (Code: {_row_detail['Party_Code']})")
        print(f"   Municipality: {_row_detail['Municipality']} (OBEC: {_row_detail['OBEC_Code']})")
        print(f"   Commission ID: {_row_detail['Commission_ID']}")
        print(f"   Commission size: {_row_detail['Commission_Size']} votes")
        print(f"   Party's share in this municipality: {_row_detail['Party_Share_in_Municipality_%']}%")
        print(f"   Party's national share: {_row_detail['Party_National_%']}%")
        print(f"   Probability of 0 votes: {_row_detail['Probability_of_Zero']}")
        print(f"   Status: {_row_detail['Status']}")
        print("-" * 100)
    return
